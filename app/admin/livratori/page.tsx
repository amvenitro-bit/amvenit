...
export default async function AdminLivratoriPage({
  searchParams,
}: {
        ...
        {!key ? (
          ...
          <div className="space-y-3">
            <p className="text-gray-700">
              Pagina admin livratori este OK (build-safe). Următorul pas este să
              afișăm cererile prin API.
            </p>

            <a
              className="inline-block rounded-full bg-orange-600 hover:bg-orange-700 px-5 py-3 font-bold text-white"
              href={`/api/admin/couriers?key=${encodeURIComponent(key)}`}
              target="_blank"
              rel="noreferrer"
            >
            ...
            <p className="text-xs text-gray-500">
              Dacă API răspunde ok, facem UI complet (approve/reject) fără să stricăm build-ul.
            </p>
          </div>
        )}
      </div>
